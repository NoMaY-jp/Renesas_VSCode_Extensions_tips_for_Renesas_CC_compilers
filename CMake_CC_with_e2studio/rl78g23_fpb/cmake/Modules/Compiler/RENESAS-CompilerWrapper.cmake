...
foreach(arg_n RANGE ${cmd_args_first} ${cmd_args_last})
  ...
  elseif(CMAKE_ARGV${arg_n} MATCHES "^-lang_cpp=(.+)")
    # This is not a compilers' native flag and it may be later removed (or, in other word, overridden).
    set(lang_cpp_arg ${arg_n})
    set(CMAKE_ARGV${lang_cpp_arg} -lang=${CMAKE_MATCH_1})
  elseif(CMAKE_ARGV${arg_n} MATCHES "^-lang=(cpp.*|.*cpp$)")
    # The first language option for C++ compiling is removed (or, in other word, overridden) here.
    if(lang_cpp_arg)
      set(CMAKE_ARGV${lang_cpp_arg} " ")
      set(lang_cpp_arg "")
    endif()
  ...
  elseif(CMAKE_ARGV${arg_n} MATCHES "^(@|-subcommand=)(.+)")
    ...
    foreach(opt ${rsp_content_list})
      ...
      if(opt MATCHES "^(-isa=|-cpu=|-Xcpu=)(.+)")
        set(target_opt ${CMAKE_MATCH_1}${CMAKE_MATCH_2})
      elseif(opt MATCHES "^-lang=(cpp.*|.*cpp$)")
        # The first language option for C++ compiling is removed (or, in other word, overridden) here.
        if(lang_cpp_arg)
          set(CMAKE_ARGV${lang_cpp_arg} " ")
          set(lang_cpp_arg "")
