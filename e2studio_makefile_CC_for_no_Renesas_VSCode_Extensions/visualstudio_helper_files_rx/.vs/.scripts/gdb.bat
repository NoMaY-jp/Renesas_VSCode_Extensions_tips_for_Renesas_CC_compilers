...
call :tool_path_setup_for_gdb %3 %4 && rem %1 is '--interpreter', %2 is 'mi' 
call :parse_launch_config_file_for_gdb %5 LCFG_ARGS
for /f "tokens=1* delims=;" %%I in ("%*") do set USER_ARGS=%%J
call :check_launch_target_program_name_for_gdb %6

cd /d "%SUPPORT_TARGET_LOC%"
rem set >>%projectRoot%\debuglog.txt && rem for debug
rem echo %GDBNAME%.exe --interpreter=mi %LCFG_ARGS% --se=%6 %USER_ARGS% >>%projectRoot%\debuglog.txt && rem for debug
%GDBNAME%.exe --interpreter=mi %LCFG_ARGS% --se=%6 %USER_ARGS%
exit %ERRORLEVEL%

:parse_launch_config_file_for_gdb
set KEY=org.eclipse.cdt.dsf.gdb.DEBUG_NAME
set VALUE=
...
for /f "usebackq tokens=3,4* delims== " %%I in ("%LAUNCH_CONFIG_FILE%") do (
    if "%%~I" == "%KEY%" (
        ...
        set VALUE=!T:~1,-3!
        set T=
        goto :parse_launch_config_file_for_gdb_search_key_break
    )
)
:parse_launch_config_file_for_gdb_search_key_break
if "%VALUE%" == "" (
    call :error 256 ^
    "The following launch file is not supported. Please contact to 'japan.renesasrulz.com/cafe_rene/f/forum21'." ^
    "'%LAUNCH_CONFIG_FILE%'"
)
rem Get command options if generated (i.e. remove command name: `%GDBNAME%` or `%GDBNAME%.exe`)
for /f "tokens=1* delims= " %%I in ("%VALUE%") do set VALUE=%%J
set %2=%VALUE%
set KEY=
set VALUE=
goto :EOF

:tool_path_setup_for_gdb
...
set PYTHONHOME=%~2
rem The following checks was done already with checks for gdbserver but do the checks here again.
if "%SUPPORT_AREA%" == "" (
    call :error 256 ^
